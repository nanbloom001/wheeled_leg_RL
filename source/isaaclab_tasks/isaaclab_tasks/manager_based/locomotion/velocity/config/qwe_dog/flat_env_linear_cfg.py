...
def track_lin_vel_x_exp(env, std: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    用于强化直线行走任务，避免 Y 轴误差干扰 X 轴的奖励计算。
    """
    asset: RigidObject = env.scene[asset_cfg.name]
    # command: [x, y, z, heading]
...
@configclass
class QweDogFlatLinearEnvCfg(LocomotionVelocityRoughEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()

        # ==========================================================
        #                 Sim-to-Real 直线行走奖励权重表 (激进版)
        # ==========================================================
        W = {
            # -------------------- 任务目标 (Task) --------------------
            # 追踪前进速度
            # 权重: 3.0 (大幅提升，这是唯一的出路)
            # STD: 0.25 (收紧宽容度，逼迫它动起来)
            "track_lin_vel_x":  3.0,      
        ...
        self.rewards.track_lin_vel_x_exp = RewTerm(
            ...
            # [关键修改] std: math.sqrt(0.25)=0.5 -> 0.25
            # 收紧奖励函数带宽，逼迫机器人动起来
...
@configclass
class QweDogFlatLinearEnvCfg_PLAY(QweDogFlatLinearEnvCfg):
    def __post_init__(self):
        super().__post_init__()
